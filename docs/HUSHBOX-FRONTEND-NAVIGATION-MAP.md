# HushBox Frontend Navigation Map

Complete navigation structure, page layouts, and URL hierarchy for HushBox.
...
| -------------- | ------------------- | ------------------------------------------------------- |
| Domain model   | Single domain       | hushbox.ai serves both marketing and app (like ChatGPT) |
| Trial access   | Limited trial       | 5 queries before signup required (like Grok)            |
...
HushBox uses two separate frontend applications deployed to the same domain:
...
\*Trial users get 5 free queries via `/chat`, then prompted to sign up.
...
└── <Outlet />
    ...
    │   ├── TrialGuard (tracks trial usage for /chat without auth)
    │   └── AppShell
...
| Trial Limit         | After 5 trial messages      | Signup prompt with value prop               |
...
// Trial route - trial access with limits
/chat          - New conversation (allow trial, cheap models, limited context, track usage)
...
| ------------- | ------------------------------- | ---------------------------------------------------------------------------------------------- |
| /chat         | conversations, messages, models | sidebarOpen, selectedModelId, pendingMessages, streamingContent, trialMessageCount             |
...
- Store `trialMessageCount` in localStorage key `hushbox-trial-token`
