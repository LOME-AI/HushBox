...
Craft has **multiple user gates**, not one. When the task needs imagery or assets, the gate sequence before code is:

1. **Shape brief confirmed** (Step 1)
2. **Direction questions answered**
3. **Palette confirmed**
4. **One visual direction approved**

You must stop at every gate. **Shape confirmation alone is NOT a green light to start coding.** It is the green light to begin landing the visual direction. Compressing gates 2 through 4 because the shape brief felt complete is the dominant failure mode of this flow.

When the task needs no separate imagery or assets, gates 2-4 collapse into the brief itself, and shape confirmation does advance straight to code.
...
- An existing framework: `vite.config.js/ts` (the app), `astro.config.mjs/ts` (marketing), or `package.json` with framework deps. **If found, use it.** Do not start a parallel build, do not introduce a second framework, do not write to `dist/` or `build/` directly. Whatever pipeline the project has, respect it.
...
What should this be built on?
  ...
  - Vite + React (when the brief implies an app surface or significant interactivity)
  - Single index.html (one-shot demo, prototype, or a deliberately framework-free experiment)
```

Default: Astro for brand briefs, Vite + React for product briefs. Ask once; don't re-ask mid-task.
...
If the task needs imagery or assets, a compact shape's "confirm or override" advances to **Step 3**, not to Step 4. Phrase the closing line accordingly: "Confirm or override; once we lock direction, I'll run a couple of palette and reference questions before settling on imagery." This stops the model from reading shape confirmation as code-green.
...
When the task needs imagery or assets, land the visual direction before code: confirm the palette, settle on a direction, and inventory the imagery the build needs. Produce or source those assets by whatever means is available (existing project assets, an image-search MCP, web-fetch, or assets the user supplies). If you need input to settle direction or sourcing, ask the user via the AskUserQuestion tool (or just ask the user directly in chat). Do not treat a built-in image generator as a precondition; if one isn't available, source imagery another way and proceed.

If the task needs no separate imagery or assets, **state in one line that the visual-direction step is being skipped because no separate assets are needed, then proceed**. The one-line announcement is required; it forces a conscious decision instead of letting the step quietly evaporate. The brief is your only visual reference. Implement directly from it, treating any named anchor references and the brief's "Design Direction" as the contract.
...
**Precondition.** If Step 3 applied (the task needs imagery or assets), its gates must be complete before any code: questions answered, palette confirmed, one direction approved, and the needed imagery produced or sourced. **Do not mention implementation, file paths, or patch plans until that's done.** A confirmed shape brief is not enough; the model that compressed those gates is the model that already failed this flow.
