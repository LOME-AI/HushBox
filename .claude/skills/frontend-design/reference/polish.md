...
4. **Pull in any prior review** (optional signal): If a prior UX review has been run on the same target, its priority issues are a useful prior for what to address first. Fold any P0/P1 items into your polish list and mention the source so the user sees what you read. A prior review is one input among many. Do your own pass either way.
