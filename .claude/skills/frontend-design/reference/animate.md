...
If any of these are unclear from the codebase, STOP and use the AskUserQuestion tool (or ask the user directly in chat) to clarify what you cannot infer.
...
```css
/* Prefer for cheap native motion: hovers, pulses, route and theme morphs */
- transitions for state changes
- @keyframes for complex sequences
- transform and opacity for reliable movement
- View Transitions API for route and theme morphs and shared-element transitions
- blur, filters, masks, clip paths, shadows, and color shifts for premium atmospheric effects when verified smooth
```

### JavaScript Animation
```javascript
/* Use for orchestrated, React-stateful animation */
- Motion for React-driven sequences, gestures, layout transitions, and spring physics
- Use it for complex choreography that CSS can't express declaratively
```
Gate every JS-driven animation through the project's motion-aware helper so it no-ops under reduced or stopped motion.
