...
- **Both exist**: STOP and use the AskUserQuestion tool (or just ask the user directly in chat) to clarify what you cannot infer. Ask which file to refresh. Skip the one the user doesn't want changed.
...
Before asking questions, thoroughly scan the project to discover what you can. This single crawl feeds both PRODUCT.md and DESIGN.md, so be thorough once rather than re-scanning later:

- **README and docs**: Project purpose, target audience, any stated goals
- **Package.json / config files**: Tech stack, dependencies, existing design libraries, **and the framework** (Vite + React for the app, Astro for marketing) plus the HTML entry the browser actually loads
...
STOP and use the AskUserQuestion tool (or just ask the user directly in chat) to clarify what you cannot infer. Ask only about what you couldn't infer from the codebase.
...
- Use the AskUserQuestion tool when available. Otherwise, ask directly in chat and stop.
...
If the signal is genuinely split (e.g. a product with a big marketing landing), STOP and use the AskUserQuestion tool (or just ask the user directly in chat) to clarify what you cannot infer. Ask which register describes the **primary** surface. The register can be overridden per task later, but PRODUCT.md carries one default.
...
Optionally STOP and use the AskUserQuestion tool (or just ask the user directly in chat) to clarify what you cannot infer. Ask whether they'd like a brief summary of PRODUCT.md appended to AGENTS.md for easier agent reference. If yes, append a short **Design Context** pointer section there.
