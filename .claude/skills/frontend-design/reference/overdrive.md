...
2. **STOP and use the AskUserQuestion tool (or ask the user directly in chat)** to present these directions and get the user's pick before writing any code. Explain trade-offs (browser support, performance cost, complexity).
...
- **Spring physics**: natural motion with mass, tension, and damping instead of cubic-bezier. Reach for Motion's spring API, or roll your own spring solver.
...
- **Motion**: JavaScript-driven, React-stateful animation for complex choreography that CSS can't express. Composable, cancellable, reversible, with gesture and layout-transition support. Gate it through the project's motion-aware helper so it no-ops under reduced or stopped motion.
