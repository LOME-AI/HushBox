...
function checkHeroEyebrow(opts) {
  ...
  // the static cascade resolves to a px length — the computed value comes
  // back as "2em" or "var(--text-9xl)" and parseFloat returns 2 or NaN. The gate fails
...
// Pure quality checks. Most run on computed CSS and node-only inputs. Two
// checks (line-length, cramped-padding) gate on element rect dimensions, which
// the static cascade can't compute (it does no layout) — pass `rect: null`
// from the static adapter to skip those.
//
...
// chain for font-size inheritance), and pass `rect: null` to skip the two
// rules that genuinely need element rects (line-length, cramped-padding).
...
function checkElementHeroEyebrow(el, style, tag, window, customPropMap) {
  ...
  // etc.) before parsing. The static cascade returns these verbatim from getComputedStyle;
  ...
  // gate falls through cleanly. The static cascade returns letter-spacing
  // verbatim (e.g. '0.15em'), unlike real browsers, so this conversion is required.
...
function checkPageTypography(doc, win) {
  ...
  // Also parse raw HTML/style content for font-family (the static cascade may not expose all of them)
  ...
  for (const el of textEls) {
    ...
    // Filter out sub-8px values (the static cascade doesn't resolve relative units to px)
