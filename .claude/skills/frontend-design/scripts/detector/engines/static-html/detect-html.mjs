...
async function detectHtml(filePath, options = {}) {
  ...
  const root = modules.parseDocument(html, { lowerCaseAttributeNames: false, lowerCaseTags: true });
  ...
  if (isFullPage(html)) {
    for (const f of checkStaticPageTypography(document, window)) {
      findings.push(finding(f.id, filePath, f.snippet));
    }
    ...
    for (const f of checkCreamPalette(document, window)) {
      findings.push(finding(f.id, filePath, f.snippet));
    }
    for (const f of checkPageQualityFromDoc(document)) {
      findings.push(finding(f.id, filePath, f.snippet));
    }
    for (const f of checkHtmlPatterns(html).filter(item =>
