...
/**
 * Shared webhook handler logic for both production and CI routes.
 */
async function handleHelcimWebhook(c: Context<AppEnv>) {
  const db = c.get('db');
...
export function createWebhooksRoutes(): OpenAPIHono<AppEnv> {
  ...
  // CI route with unique run ID to isolate from stale webhook retries
  // This must be registered before the OpenAPI route to take precedence
  app.post('/payment/:runId', handleHelcimWebhook);

  // Production route (documented in OpenAPI)
  app.openapi(helcimWebhookRoute, handleHelcimWebhook);
