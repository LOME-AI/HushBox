...
describe('CapacityBar', () => {
  describe('rendering', () => {
    it('renders with correct testid', () => {
      render(<CapacityBar currentUsage={1000} maxCapacity={10000} />);
    ...
    it('rounds percentage to whole number', () => {
      render(<CapacityBar currentUsage={3333} maxCapacity={10000} />);
      // 3333/10000 = 33.33% -> rounds to 33%
      expect(screen.getByText('Model 33% filled')).toBeInTheDocument();
    });

    it('accepts custom className', () => {
      render(<CapacityBar currentUsage={1000} maxCapacity={10000} className="custom-class" />);
  ...
  describe('color zones', () => {
    it('shows green fill when below 33%', () => {
      render(<CapacityBar currentUsage={3200} maxCapacity={10000} />);
      // 32% should be green
    ...
    it('shows yellow fill at exactly 33%', () => {
      render(<CapacityBar currentUsage={3300} maxCapacity={10000} />);
      const fill = screen.getByTestId('capacity-bar-fill');
      expect(fill).toHaveClass('bg-yellow-500');
    });

    it('shows red fill when at or above 67%', () => {
      render(<CapacityBar currentUsage={6700} maxCapacity={10000} />);
      // 67% should be red
      const fill = screen.getByTestId('capacity-bar-fill');
    ...
    it('shows red fill when over 100%', () => {
      render(<CapacityBar currentUsage={15000} maxCapacity={10000} />);
      // 150% should be red
      const fill = screen.getByTestId('capacity-bar-fill');
      expect(fill).toHaveClass('bg-red-500');
    });
  ...
  describe('fill width', () => {
    ...
    it('caps fill width at 100% when over capacity', () => {
      render(<CapacityBar currentUsage={15000} maxCapacity={10000} />);
      const fill = screen.getByTestId('capacity-bar-fill');
    ...
    it('shows 0% width when currentUsage is 0', () => {
      render(<CapacityBar currentUsage={0} maxCapacity={10000} />);
      const fill = screen.getByTestId('capacity-bar-fill');
