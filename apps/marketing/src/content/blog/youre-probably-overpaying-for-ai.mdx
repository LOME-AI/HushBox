...
HushBox charges per message based on the exact API cost of the model you use, plus a 9.5% fee. That fee breaks down as 5% for HushBox's margin and 4.5% for credit card processing. A separate storage fee of $0.0003 per thousand characters covers keeping your conversations encrypted. Credits never expire. Minimum deposit is $5.

A concrete month: you send 20 messages a day using Claude Sonnet 4.6, with typical conversation lengths. Each message costs roughly 1.2 cents at API rates plus the 9.5% fee. Over 30 days, that's 600 messages for about $7. Storage adds a few cents.
...
At HushBox, you pay a 9.5% fee on the exact cost of each message. No subscription. No monthly commitment. Five dollar minimum deposit. For most people, that works out to a few dollars a month, not twenty.
